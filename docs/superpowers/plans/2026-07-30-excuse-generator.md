...
> ⚠️ **Код в этой задаче устарел после финального ревью.** Показанная ниже версия
> `getKv()` при отсутствии переменных Upstash молча откатывается в память процесса.
> Ревью нашло это как Critical: на продакшене это отключает и рейт-лимит, и дневной
> бюджет, оставляя публичный платный эндпоинт без предохранителя. **Действующий код в
> `src/lib/kv.ts` бросает исключение при `NODE_ENV === "production"`**, а Route Handler
> перехватывает его и отдаёт 503. Там же появилась `resetKvCacheForTests()`, которой в
> этой задаче нет. Источник истины — код и
> `docs/superpowers/plans/2026-07-30-session-handoff-tasks-9-10.md`.
...
> ⚠️ **Код в этой задаче устарел после финального ревью.** В показанной ниже версии вызов
> `checkLimits(getKv(), ...)` не обёрнут в `try/catch`, поэтому отказ хранилища уходил
> наружу необработанным исключением вместо контрактного тела `{ error }`. **Действующий
> код в `src/app/api/generate/route.ts` оборачивает его** и отдаёт 503 с текстом
> `ERROR_MESSAGES.budget_exhausted`, не делая платного вызова. Также изменился разбор
> адреса клиента: приоритет отдан заголовку `x-vercel-forwarded-for`, который клиент
> подделать не может.
